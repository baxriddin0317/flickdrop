...
const InventoryAndSkins = ({title, items}: props) => {
  ...
  return (
    <div className="bg-brand-dark rounded-3xl overflow-hidden w-full">
      <div className="px-6 py-2.5 h-[66px] flex items-center justify-between border-b border-brand-gray-2">
        <h3 className="text-brand-gray-2 text-ellipsis font-medium text-lg line-clamp-1">{title}</h3>
        {items.length > 0 && (
          <div className="flex items-center gap-2 ">
            <div className="w-[184px] h-11 rounded-[10px] bg-brand-neutral-4 border border-white/25 shadow-[0_0_14.721px_0_rgba(0,0,0,0.06)_inset] px-3 py-1.5 flex flex-col justify-between">
              <div className="flex items-center justify-between text-[11px] font-medium tracking-wide text-brand-gray-2">
            ...
            <div className="relative overflow-hidden w-[254px] h-11 rounded-[10px] bg-brand-neutral-4 border border-white/25 shadow-[0_0_14.721px_0_rgba(0,0,0,0.06)_inset] ">
              ...
              <input className="bg-transparent outline-none absolute size-full pl-9 text-white placeholder:text-brand-gray-2 text-sm font-medium" type="text" placeholder="Search" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-6 pr-3">
        {items.length > 0 ? (
          <div className="overflow-y-scroll scroll-style grid grid-cols-5 gap-1 h-[533px]">
            {items.map((item) => (
              <SkinCard key={item.id} price={item.price} name={item.name} type={item.type} />
            ))}
          </div>
        ) : (
          <div className="h-[533px] flex items-center justify-center gap-8 flex-col">
            <p className="text-brand-gray-2 text-lg font-medium text-center">
            You don't have any items available yet. <br />
            Open cases and upgrade the item
            </p>
            <button 
              className="px-7 py-4 uppercase font-bold btn cursor-pointer" 
              type='button'
            >
              OPEN cases
            </button>
...
export default InventoryAndSkins